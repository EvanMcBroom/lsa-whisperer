...
namespace Rpc {
    ...
    bool Client::Bind(RPC_BINDING_HANDLE* binding) {
        RpcTryExcept auto address{ (server.length()) ? reinterpret_cast<RPC_WSTR>(server.data()) : nullptr };
        ...
        RpcExcept(EXCEPTION_EXECUTE_HANDLER)
                std::wcerr
            << L"Could not connect to: " << reinterpret_cast<LPWSTR>(this->stringBinding);
        RpcEndExcept return bound;
    }
