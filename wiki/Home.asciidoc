...
A client will need to specify a package ID when making this call which may be obtained with `LsaLookupAuthenticationPackage`.
A client may alternatively use a special package ID, `SECPKG_ALL_PACKAGES` or `-2`, which LSA will handle by issuing the call to all loaded APs.
There is a small set of calls that are intended for this feature which each AP may choose to process or ignore.
These calls and the APs that currently support them are provided here.
