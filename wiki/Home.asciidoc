...
Additionally, the components for the distributed computing environment (DCE), distributed password authentication (DPA), kernel, and message queue (MQ) RPC authn options could not be identified for auditing.
