...
The call has been put in scope to facilitate reusing a NT OWF hash on a user controlled VM with https://learn.microsoft.com/en-us/windows-server/security/credentials-protection-and-management/configuring-additional-lsa-protection#disable-lsa-protection[LSASS configured to not start as a PPL process].
...
The command is restricted to only being callable by LSASS itself and as such was originally outside the scope of LSA Whisperer.
The call has been put in scope to assist with using the `CacheLogon` call.
